...
queue:false,complete:function(){b.browser.msie&&this.style.removeAttribute("filter")}})}).bind("panel-close.accordion",function(){var d=b(this);d.closest("li").removeClass(a.activeClassLi);d.removeClass(a.activeClassPanel).css({overflow:"hidden"}).animate(b.browser.msie&&parseInt(b.browser.version)<8?f:b.extend({opacity:0},f),{duration:a.speed,easing:a.easing,queue:false,complete:function(){b.browser.msie&&this.style.removeAttribute("filter");d.hide()}})}).hide().css("visibility","visible");return c});
e.find(" > li > "+a.handle).wrapInner('<a class="accordion-opener" href="#open-panel" />');e.find(" > li."+a.activeClassLi+" > "+a.panel+", > li."+a.lockedClass+" > "+a.panel).show().addClass(a.activeClassPanel);var j=e.find(" > li."+a.activeClassLi+", > li."+a.lockedClass);!a.canToggle&&j.length<1&&e.find(" > li").first().addClass(a.activeClassLi).find(" > "+a.panel).addClass(a.activeClassPanel).show();e.delegate(".accordion-opener","click",function(c){c.preventDefault();c.stopImmediatePropagation();
